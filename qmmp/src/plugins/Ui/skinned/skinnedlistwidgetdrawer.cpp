...
void SkinnedListWidgetDrawer::readSettings()
{
    ...
    if(!settings.value("pl_use_skin_colors"_L1, true).toBool())
    {
        ...
        m_alternate_bg = QColor(settings.value("pl_bg2_color"_L1, m_alternate_bg.name()).toString());
        ...
        m_highlighted = QColor(settings.value(u"pl_hl_text_color"_s, m_highlighted.name()).toString());
        m_splitter = QColor(settings.value("pl_splitter_color"_L1, m_splitter).toString());
        m_group_text = QColor(settings.value("pl_group_text"_L1, m_group_text.name()).toString());
#else
    ...
    {
#if QT_VERSION >= QT_VERSION_CHECK(6, 4, 0)
        m_group_bg = QColor(settings.value("pl_group_bg"_L1, m_normal_bg.name()).toString());
#else
